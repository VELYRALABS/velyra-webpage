    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? "bg-[#FAF9F6]/85 backdrop-blur-xl border-b border-black/5 py-3"
            : "bg-transparent py-5"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <Link href="/">
            <div className="flex items-baseline gap-1.5 cursor-pointer">
              <span className="font-display text-[15px] font-semibold tracking-[-0.5px] text-[#1A1A2E]">
                PROJECT LYRA
              </span>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href}>
                <span className={`text-sm font-medium transition-colors cursor-pointer ${pathname === link.href ? 'text-primary' : 'text-[#64748B] hover:text-[#1A1A2E]'}`}>
                  {link.name}
                </span>
              </Link>
            ))}
          ...
          >
            ...
            <div className="flex-1 flex flex-col px-8 py-8 gap-6">
              {navLinks.map((link) => (
                <Link key={link.name} href={link.href}>
                  <span 
                    className="text-3xl font-display font-semibold text-[#1A1A2E]"
